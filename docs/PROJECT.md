...
| Magic-link errors swallowed by www redirect | Bug | — | After domain split, failed `/auth/confirm` → `app/?error=…` was 308'd to www with query stripped; users saw a blank homepage and re-requested links. Fixed: redirect failures to `/tailor?error=…`, preserve auth params in proxy, accept PKCE `code` on confirm |
