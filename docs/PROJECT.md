...
| Item | Type | PR | Notes |
|------|------|----|-------|
| **B2B maps vs the staging agency product — reconciliation** | Doc | — | 10 Sep. `docs/b2b-maps-vs-staging.html`, published. Diffs the 9 Sep maps against `origin/staging` @ `8d0f45f` (45 agency modules, 110 API routes, 21 screens, 44 agency/matching test files). **Headline: the maps modelled the wrong shape** — they assumed an in-house employer buying a screening tool; the build is for recruitment agencies, with the employer as the agency's client. 3 of 9 workflow stages agree (incl. the must-have gate); screening calls and the interviews/handover phases were never mapped. Five of the six open decisions were already settled in code — "no auto-reject" is enforced with a guardrail test, candidate consent is opt-in with a disclosure manifest. Two live risks recorded: two scoring engines on different scales, and main/staging migration divergence (`019` missing from main). No code, no migration |
| **Tailr B2B (employers & recruiters) — process maps, persona journeys, flows** | Doc | — | 9 Sep. `docs/b2b-employer-journeys.html`, published for Ose's review. Two process maps (the req→hire operating loop; the commercial lifecycle with four stop-gates), five personas (TA lead, recruiter, hiring manager, People Ops/DPO, candidate) each with a journey map and a flow. Grounded in what already exists: `RequirementMapping`, `computeMatchScore`, the Evidence Rule. Proposes a second product law — **Tailr ranks and evidences; a person decides; nothing is ever auto-rejected**. Six decisions open for Ose (see §06). No code, no migration. **Notion card still outstanding** — connector dropped mid-session |
