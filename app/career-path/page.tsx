...
function NorthStarJourney({ cachedFindings, seedIntention, onBuilt, onCancel }: { cachedFindings: CvFindings | null; seedIntention: string; onBuilt: () => Promise<void> | void; onCancel?: () => void }) {
  ...
  const build = async (role: string) => {
    ...
    try {
      ...
      // Show the researched path immediately. LivingPath owns enrichment so
      // failed or partial batches retry on the next visit instead of being
      // lost in a fire-and-forget request from this component as it unmounts.
      await onBuilt()
    } catch (err) {
            ...
            {miss.map((sk, i) => {
              ...
              return (
                <button key={i} disabled={isAdding}
                  ...
                  {sk}
                  {/* Every researched North Star skill is core to the role —
                      tag them all rather than trusting the model's split. */}
                  <span className="t-mono" style={{ fontSize: 9.5, color: "var(--ns-coral-deep)", letterSpacing: "0.08em" }}>CORE</span>
                </button>
              )
            })}
          </div>
...
function LivingPath({ data, reload, onChangeTarget }: { data: PathData; reload: () => Promise<void>; onChangeTarget: () => void }) {
  ...
  // The forecast is computed client-side, so a pace change must not wait for
  // the server round-trip + full reload — show it instantly, save behind it.
  const [pace, setPace] = useState<number | null>(roadmap.hours_per_week)
  useEffect(() => { setPace(roadmap.hours_per_week) }, [roadmap.hours_per_week])
  const enrichmentStarted = useRef(false)
  const needsEnrichment = roadmap.items.some(
    (item) => (item.resources ?? []).length === 0 && !item.projectBrief,
  )

  useEffect(() => {
    if (!needsEnrichment || enrichmentStarted.current) return
    ...
    const enrichAll = async () => {
      let remaining = 1
      let batches = 0
      // Role research returns at most 14 skills and enrichment handles five
      // per model call. Four batches leaves headroom without an unbounded cost
      // loop if malformed output never fills a placeholder.
      while (!cancelled && remaining > 0 && batches < 4) {
        const result = await readJson<{ remaining?: number }>(
          await fetch("/api/career-path", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ mode: "enrich-plan" }),
          }),
        )
      ...
      if (!cancelled && remaining > 0) {
        toast.error("Some skill plans are still incomplete. Tailr will retry when you next open your path.")
      }
    }

    enrichAll().catch((err) => {
      if (!cancelled) {
        toast.error(err instanceof Error ? err.message : "Couldn't finish the learning plans. Tailr will retry next time.")
      }
    })
    return () => {
      cancelled = true
      // React Strict Mode replays effects in development. Reset the guard so
      // the replayed setup, rather than the deliberately cancelled one, runs.
      enrichmentStarted.current = false
    }
  }, [needsEnrichment, reload])
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between" style={{ gap: 40, marginBottom: 48 }}>
          ...
          <div style={{ padding: "18px 22px", background: "var(--ns-paper)", border: "1px solid var(--ns-border)", borderRadius: 12, minWidth: 280 }}>
            ...
            {(() => {
              ...
              const f = forecastReadyDate(open, pace)
                    ...
                    <p className="t-body" style={{ margin: 0 }}>
                      ...
                      <select
                        ...
                        onChange={async (e) => {
                          ...
                          setPace(next)
                          try {
                            await readJson(await fetch("/api/career-path", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "set-pace", hoursPerWeek: next }) }))
                          } catch (err) {
                            setPace(previous)
                            toast.error(err instanceof Error ? err.message : "Couldn't update your pace.")
                          }
                        }}
